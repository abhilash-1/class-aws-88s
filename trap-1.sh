...
trap ' There is an error at line number $LINENO, and command: $BASH_COMMAND | tee -a "Logs_file" ' ERR

validate(){
    set +e
    if [ $1 -ne 0 ]; then
        
        echo "$R $2: Installation falied" | tee -a $Logs_file
    else
        echo "$G $2: Installation success" | tee -a $Logs_file
    fi
    set -e
}
